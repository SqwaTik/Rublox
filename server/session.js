...
const KNOWLEDGE_RULES =
  ...
  '- САМООБУЧЕНИЕ: когда понял что-то полезное на будущее (рабочий приём, как устроена ' +
  'конкретная карта/механика, неочевидная грабля и как её обойти) — вызывай remember(text) ' +
  'и сохраняй это кратко. В начале новых задач опирайся на блок «ВЫУЧЕННОЕ», не переоткрывай ' +
  'уже известное. Если пользователь подключил мост и просит изучить место — capture_place ' +
  'снимет слепок плейса в память.';
...
const BUILD_RULES =
  ...
  '- ПЕРЕДЕЛКА КАРТЫ: если просят «переделай/перестрой/сделай заново карту/уровень» — ' +
  'это НЕ точечная правка. Сначала изучи текущую сцену (get_studio_context), затем ОБЯЗАТЕЛЬНО ' +
  'заново вызови plan_build (новый генплан вида сверху) и только потом строй по нему. Не ' +
  'правь старое вслепую и не пропускай генплан — без plan_build переделка считается невыполненной.\n' +
  '- ИСПОЛЬЗУЙ ГОТОВЫЕ МОДУЛИ, НЕ ГЕНЕРИРУЙ ГЕОМЕТРИЮ КОДОМ (run_code) — ручной код даёт кривые ' +
  'лестницы/двери/деревья. Лестница → build_stairs (ровные ступени). Пол/потолок/плита/дорога → ' +
  'build_floor. Крыша → build_roof. Колонны/столбы → build_pillar. Забор/ограда → build_fence. ' +
  'Дерево → build_tree (или ассет из тулбокса через search_assets, НЕ кодом). Открывающаяся дверь → ' +
  'build_room с doorways[].door=true. Окна (с подоконником и стеклом) → build_room windows[]. ' +
  'run_code — только для логики/скриптов, не для построек.\n' +
  '- ПОЛ В ПОМЕЩЕНИИ — это пол (дерево/бетон/плитка), НЕ трава. Траву (Grass) клади только на ' +
  ...
  '- ПЕСОЧНИЦА для качества: для ответственных построек строй через sandbox_build (open → строй в выданный ' +
  'parent → inspect → чини → commit). Так проверяешь до установки в сцену; discard если не вышло.\n' +
  '- ПОМЕТКИ ПОЛЬЗОВАТЕЛЯ: если просят «отметить что не так» — set_inspect_mode enabled=true, пусть кликает ' +
  'по местам в Studio, затем get_user_markers и исправь рядом с каждой меткой; в конце выключи режим.\n' +
  '- ЭСТЕТИКА (для «красивых» построек): держи единую палитру 3–5 цветов; добавляй ОБВОДКУ/карнизы ' +
  '(тонкие парты по краям), цоколь/основание, скруглённые края через WedgePart/клинья; чередуй материалы ' +
  '(не всё SmoothPlastic); ставь источники света (PointLight/SpotLight) у ламп; добавляй мелкие детали ' +
  '(наличники, балки, перила, растения). Симметрию держи там, где она уместна.\n' +
  '- ПОКАЖИ РЕЗУЛЬТАТ: после крупной постройки/модели вызови preview_in_3d на ней — пользователь увидит её ' +
  'в крутящемся 3D-окне (сетка-пол, свет, поворот мышью/зум) и сразу оценит вид, не бегая по сцене.\n' +
  '- ДЕТАЛЬНЫЕ/ОРГАНИЧНЫЕ объекты (статуи, мебель, оружие, декор, существа) лучше НЕ собирать из партов и ' +
  'НЕ искать в тулбоксе, а сгенерировать через generate_3d_model (text→3D-меш Tripo/Meshy) и вставить — ' +
  'выходит детальнее. Постройки/каркас зданий — партами/модулями; детали и реквизит — генерацией меша.';
...
const STUDIO_PROMPT =
  ...
  '3D-МОДЕЛИ И ПРОСМОТР: generate_3d_model — сгенерировать модель по тексту (AI-меш Meshy → ' +
  'загрузка в Roblox → вставка как MeshPart; prompt пиши по-английски, polycount 3000–8000). ' +
  'preview_in_3d — показать любой Model/BasePart пользователю в крутящемся 3D-окне (после ' +
  'постройки/генерации покажи результат). insert_mesh_asset — вставить меш по assetId. ' +
  'Если generate_3d_model жалуется на ключи — скажи пользователю настроить их (Meshy + Open Cloud).\n' +
  ...
  '(анимации Humanoid).\n' +
  'ПРАВИЛА КАТСЦЕН (иначе выходит криво):\n' +
  '- НЕ угадывай координаты камеры из головы. Сначала get_studio_context/find_instances, ' +
  'возьми реальную позицию объекта/NPC, и наводись через frame.focus (путь к объекту), а на ' +
  'персонажа — frame.focus + frame.focusFace:true (камера встанет ПЕРЕД лицом, а не за спиной).\n' +
  '- 3–6 кадров. duration 2–4 сек, easingStyle "Sine". fov 45–70 (меньше = драматичнее), не ' +
  'уезжай ниже 30. shake только в напряжённых кадрах (0.2–0.5). hold 0.5–1.5 сек на ключевых.\n' +
  '- Между кадрами меняй ракурс ОСМЫСЛЕННО (общий план → средний → крупный на лицо), не дёргай ' +
  'камеру резко на 180°. Фейды/леттербокс оставляй включёнными.\n' +
  '- АНИМАЦИИ Humanoid (frame.anim / play_animation): НИКОГДА не выдумывай animationId. Найди ' +
  'реальный через search_assets (assetType "animation"/"model"), проверь что это твоя анимация, ' +
  'и только тогда подставляй rbxassetid://. Несуществующий id = пустой кадр.\n' +
  '- ПРЕСЕТЫ (отталкивайся от них): ДИАЛОГ — 2 NPC, чередуй focus+focusFace на говорящего, ' +
  'fov 50–60, duration 2.5–3.5, hold 1–1.5, лёгкий shake 0; ОБЛЁТ объекта — общий план с ' +
  'высоты → плавный круговой проход вокруг (focus на объект), fov 60–70; ЭКШЕН — короче кадры ' +
  '(1.5–2.5), fov 40–50, shake 0.3–0.5, flash на удар/взрыв, музыка-напряжение.\n' +
  ...
  'UI/ИКОНКИ: НИКОГДА не вставляй эмодзи (🚗📏⚡💰🏁 и т.п.) в текст TextLabel/TextButton и в ' +
  'код — это выглядит дёшево. Вместо эмодзи используй ВЕКТОРНЫЕ иконки: ImageLabel/ImageButton ' +
  'с векторной иконкой (подбери asset id через search_assets assetType="image"/"decal", напр. ' +
  'набор иконок), либо чистый текст без значков. Подписи делай словами, иконку — отдельным ' +
  'ImageLabel рядом.\n' +
...
// Правила памяти/контекста — особенно важны после разрыва связи со Studio:
// модель должна опираться на историю чата, а не дублировать уже отправленное.
const MEMORY_RULES =
  'ПАМЯТЬ И КОНТЕКСТ: история этого чата — твоя память. Всё, что ты уже создавал, ' +
  'писал и говорил выше, остаётся в переписке и пользователю видно. НЕ вставляй ' +
  'заново целые скрипты/файлы, которые уже отправлял в этом чате — ссылайся на них ' +
  'коротко («скрипт из сообщения выше»), показывай только изменённые фрагменты. ' +
  'Если соединение со Studio разорвано, а нужно применить правку в игре — попроси ' +
  'нажать Connect в плагине Rublox и продолжи, НЕ заставляй копировать код вручную, ' +
  'если он уже есть в чате. Помни, что было сделано до разрыва, и продолжай с того места.';
...
const PC_PROMPT =
  ...
  '- git — ПОЛНЫЙ: status/diff/log/branch и add/commit/push/pull/checkout/merge/' +
  'stash/clone и любая другая подкоманда (action+args). launch_app, focus_window, ' +
...
export class Session {
  ...
  systemPrompt({ studioConnected = false, pcAllowed = false } = {}) {
    ...
    // При подключённом Studio ПК-инструменты тоже могут быть доступны (по запросу).
    if (studioConnected && pcAllowed) {
      parts.push('ПК-ИНСТРУМЕНТЫ ДОСТУПНЫ И СЕЙЧАС: помимо Studio тебе доступны инструменты ПК ' +
        '(run_command, git, read_file/write_file/edit_file, run_code_sandbox и т.п.). ' +
        'Если пользователь просит сделать что-то на компьютере — делай через них, не отключая Studio.');
    }
    parts.push(MEMORY_RULES, FORMAT_RULES, INTERACTION_RULES, KNOWLEDGE_RULES, ECONOMY_RULES);
    let p = parts.join('\n\n');
    ...
    if (proj) p += `\n\n${proj}`;
    // Накопленный опыт (самообучение) — общий для всех чатов.
    const know = knowledgePromptBlock();
    if (know) p += `\n\n${know}`;
  ...
  addAssistant(text, toolCalls, thinking) {
    // thinking — сырые thinking-блоки Anthropic (с подписью). Их обязательно
    // возвращать в API при extended thinking + tool_use, иначе 400.
    this.messages.push({ role: 'assistant', content: text || '', toolCalls: toolCalls || [], thinking: thinking || [] });
  }
